...
rerunning once per frame. The loop caps itself so a forgotten tab does not burn
a core forever.
"""
...
MAXIMUM_FRAMES_PER_RUN: int = 300
FRAME_INTERVAL_S: float = 0.12
CELLULAR_AUTOMATON_BURN_DURATION_S: float = 10.0
RATE_OF_SPREAD_DEFAULT_TIME_STEP_S: float = 10.0
CELLULAR_AUTOMATON_DEFAULT_TIME_STEP_S: float = 1.0
...
def compute_default_time_step_s(
    config: ForwardSimulationConfiguration, context: SimulationContext
) -> float:
    """Pick a frame timestep that resolves the front for the chosen engine.
    ...
    if config.fire.spread_engine_name != RATE_OF_SPREAD_ENGINE:
        return CELLULAR_AUTOMATON_DEFAULT_TIME_STEP_S
...
def start_simulation(
    config: ForwardSimulationConfiguration, requested_time_step_s: float | None
) -> LiveSimulation:
    """Build every component and ignite the configured points.

    Args:
        config: The configuration the widgets assembled.
        requested_time_step_s: Frame timestep, or None to derive one.
    ...
    return LiveSimulation(
        ...
        time_step_s=(
            compute_default_time_step_s(config, context)
            if requested_time_step_s is None
            else requested_time_step_s
        ),
    )
...
def describe_frame(simulation: LiveSimulation) -> str:
    ...
    return (
        ...
        f"burning {int(np.count_nonzero(simulation.state.is_burning))}  ·  "
...
def render_parameter_controls(
    ...
    Returns:
        The parameter values, and the requested frame timestep or None.
    """
    ...
    engine_column, ignition_column, step_column = st.columns(3)
    with engine_column:
        st.markdown("**Engine**")
    ...
    with step_column:
        st.markdown("**Time**")
        use_automatic_time_step = st.checkbox("derive timestep from CFL", value=True)
        requested_time_step_s = (
            None
            if use_automatic_time_step
            else st.slider(
                "seconds per frame", 0.5, 60.0, RATE_OF_SPREAD_DEFAULT_TIME_STEP_S, 0.5
            )
        )
...
def render() -> None:
    ...
    base_label = st.selectbox(
        "start from",
        list(scene_options) or ["configs"],
        index=0,
        key="simulate_base_scene",
    )
    base = load_configuration(str(scene_options.get(base_label, Path("configs"))))

    parameters, requested_time_step_s = render_parameter_controls(base)
    ...
    if simulation is None:
        st.info("Set the parameters above, then press Build.")
        return

    st.caption(
        f"dt = {simulation.time_step_s:.1f} s per frame  ·  "
    ...
    for _ in range(MAXIMUM_FRAMES_PER_RUN):
        simulation.state = simulation.context.spread_engine.step(
            simulation.state, simulation.time_step_s
        )
        simulation.frame_index += 1
